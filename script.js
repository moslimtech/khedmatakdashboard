...
// تعديلات واجهة المستخدم (script.js) - إضافة فحص حصة الإعلانات وعرض/إخفاء زر إضافة الإعلان
// تأكد أن API_URL صحيح ويوجّه إلى رابط Web App /exec
const API_URL = 'https://script.google.com/macros/s/AKfycbx-fMI2hsJ5LvKKh9fzd3Vidn2TeGtEbHV9Nyj2nZBy9xQk9Uy_uL-m3hrDqp1uUWAPwA/exec';

// ... (ابقى كودك السابق كما هو) ...

// أضف هذه الدالة لفحص الحصة وتبديل واجهة الإعلان:
async function checkAdQuotaAndToggle(placeId) {
  try {
    if (!placeId) {
      // لا مُستخدم مسجل/لا مكان معروف -> إخفاء تبويب الإعلان
      document.getElementById('tab-ads').style.display = 'none';
      return;
    }
    const url = `${API_URL}?action=remainingAds&placeId=${encodeURIComponent(placeId)}`;
    const res = await fetch(url);
    const json = await res.json().catch(()=>null);
    if (!json || !json.success) {
      // في حالة خطأ: عرض التبويب لكن تعطيل زر الحفظ حفاظًا على الأمان
      document.getElementById('tab-ads').style.display = 'block';
      toggleAdFormAllowed(false, 'تعذر التحقق من حصة الإعلانات');
      return;
    }
    ...
    // عرض التبويب (المستخدم مسجل)
    document.getElementById('tab-ads').style.display = 'block';
    if (remaining > 0) {
      toggleAdFormAllowed(true);
      // خيار: إظهار عدد المتبقي في واجهة الباقات أو بالقرب من زر الحفظ
      showAdQuotaMessage(`الإعلانات المسموح بها: ${allowed} · المستخدمة: ${used} · المتبقي: ${remaining}`);
    } else {
      toggleAdFormAllowed(false, 'لقد استنفدت عدد الإعلانات المسموح به في باقتك');
      showAdQuotaMessage(`الإعلانات المسموح بها: ${allowed} · المستخدمة: ${used} · المتبقي: ${remaining}`);
    }
  } catch (err) {
    console.error('checkAdQuota failed', err);
    document.getElementById('tab-ads').style.display = 'block';
    toggleAdFormAllowed(false, 'خطأ أثناء التحقق من الحصة');
  }
}
...
function toggleAdFormAllowed(allowed, message) {
  ...
  const adNotice = document.getElementById('adQuotaNotice');
  if (adNotice) {
    adNotice.textContent = message || '';
    adNotice.style.display = message ? 'block' : 'none';
  } else if (!allowed && message) {
    // إن لم تكن هناك منطقة إشعار، أنشئ واحدة تحت نموذج الإعلان
    const formContainer = document.getElementById('ads-tab');
    if (formContainer) {
      const div = document.createElement('div'); div.id = 'adQuotaNotice';
      div.style.background = '#fff3cd'; div.style.color = '#856404'; div.style.padding = '10px'; div.style.borderRadius = '6px';
      div.style.marginTop = '12px'; div.textContent = message;
      formContainer.insertBefore(div, formContainer.firstChild.nextSibling);
    }
  }
}
...
// دَع checkAdQuotaAndToggle يُستدعى بعد تسجيل الدخول
// مثال: داخل setLoggedInUI بعد قفل select المكان أضف:
 // const logged = getLoggedPlace();
 // if (logged && logged.id) checkAdQuotaAndToggle(logged.id);

// وأيضاً بعد تحميل القوائم/المواقع لأجل حالة الجلسة عند بدء الصفحة نادِ:
 // const stored = getLoggedPlace(); if(stored && stored.id) checkAdQuotaAndToggle(stored.id);

// بعد حفظ إعلان بنجاح يمكنك إعادة فحص الحصة لتحديث الواجهة:
 // await saveAdToSheet(...); await checkAdQuotaAndToggle(placeId);

// ملاحظة: تأكد أن script.js لديك يستدعي checkAdQuotaAndToggle بعد setLoggedInUI و loadPlacesForAds.
// انتهى التعديل اللازم في واجهة المستخدم.

...
function showPaymentModal(packageId) {
  const pkg = (window.availablePackages || []).find(p => String(p.id) === String(packageId));
  const methods = window.availablePaymentMethods || [];
  let html = '';

  if (pkg) {
    html += `<p><strong>الباقة المختارة:</strong> ${pkg.name}</p>`;
    html += `<p><strong>المبلغ المطلوب:</strong> ${pkg.price || pkg.raw?.['السعر'] || '—'} ج.م</p>`;
  }

  if (methods.length > 0) {
    html += '<h4>طرق الدفع المتاحة:</h4><ul style="padding-right:18px">';
    methods.forEach(m => {
      html += `<li><strong>${m.name}</strong> — ${m.id}</li>`;
    });
    html += '</ul>';
  } else {
    html += '<p>لا توجد طرق دفع مسجلة حالياً</p>';
  }
...
async function confirmPayment() {
  try {
    ...
    // إرسال لوج الدفع للشيت
    const payload = {
      action: "logPaymentNotice",
      placeId: logged.id,
      placeName: logged.name,
      packageId: logged.raw?.['الباقة'] || '',
      timestamp: new Date().toISOString()
    };
    const resp = await apiPost(payload);
    if (!resp.ok) throw new Error("تعذر تسجيل الدفع");
    showSuccess("تم تسجيل عملية الدفع، سيتم تفعيل الباقة بعد المراجعة");
  } catch (err) {
    showError(err.message || "خطأ أثناء تسجيل الدفع");
  } finally {
    closePaymentModal();
  }
}
